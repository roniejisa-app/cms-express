module.exports = {
    ...
    recursiveHTMLFolder: (folders, idActives = [], path = "", level = 0) => {
        if (!Array.isArray(folders) || (Array.isArray(folders) && folders.length === 0)) {
        ...
        folders.forEach(folder => {
            ...
            output += `<a href="/admin/medias/${path}" class="${idActives.length ? +idActives[idActives.length - 1] === folder.id ? 'active' : '' : ''}">`;
            ...
            output += `</a>`
            if (folder.children && folder.children.length && folder.children.some(folderChild => idActives.some(id => +id === +folderChild.id))) {
                level++;
                output += module.exports.recursiveHTMLFolder(folder.children, idActives, path, level);
            }
            output += `</li>`;
        })
        output += "</ul>";
            ...
            if (folder.media_id === null) {
                ...
                newAsideFolders.forEach(folderNew => {
                    if (folder.media_id === folderNew.id) {
                        ...
                        module.exports.recursiveFolderChild(folderNew.children, newFolder)
                    }
                })
            }
        })
