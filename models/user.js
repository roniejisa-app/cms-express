    ...
    class User extends Model {
        ...
        static validate(id = null) {
            let validate = {
                ...
                email: string().required("Vui lòng nhập email").email("Không đúng định dạng email").test('check-email', "Email đã tồn tại", async (value) => {
                    const result = id ? await User.findOne({
                        where: {
                            email: value,
                            id: {
                                [Op.not]: id
            ...
            if (!id) {
                validate.password = string().min(6, 'Mật khẩu phải có ít nhất 6 ký tự').required('Vui lòng nhập mật khẩu');
            }
            return validate;
        }
    }
