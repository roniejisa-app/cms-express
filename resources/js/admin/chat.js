...
import {
    createScrollbar,
    resizeEditorChat,
    beforeResizeEditorChat,
} from '../utils/scroll-bar.js'
...
const CHAT = (() => {
    ...
    function addSocketEvent() {
        ...
        socket.on('feel-message-response', (value) => {
            ...
            const lastMessage =
                messageEl.children[messageEl.children.length - 1]
    ...
    async function handlePasteData(e) {
        ...
        let item = Array.from(e.clipboardData.items).find((x) =>
            /^image\//.test(x.type)
        )
    ...
    function itemUploadMessage(imageEl, file) {
        ...
        closeEl.onclick = () => {
            const index = Array.from(newDataTransfer.files).findIndex(
                (file) => file.customId === info.customId
            )
            if (index !== -1) {
        ...
        Array.from(messageEl.children).forEach((messageItem) => {
            ...
            feelMart.onclick = (e) => {
                ...
                setTimeout(() => {
                    ...
                    picker.style.left =
                        e.pageX - rect.width - feelMart.offsetWidth + 'px'
    ...
    function templateMessage(response, isMe, messageEl) {
        ...
        const feel =
            data.feels && data.feels.length > 0
                ? data.feels[data.feels.length - 1]
                : null
        ...
        if (
            lastItem &&
            lastItem.classList.contains('left') &&
            lastItem.dataset.id === user.socketId &&
            lastItem.querySelector('.avatar img')
        ) {
            lastItem.querySelector('.avatar img').remove()
        }
        const content = getContentChat(data, feel)
        return `<div class="item-message ${isMe ? 'right' : 'left'}" data-id=${
            data.id
        }>
            ${
                isMe
        ...
        switch (data.type) {
            ...
            case 'sticker':
                return `<div class="message-body sticker">${data.message}
                        ${
                            feel
                                ? `<span class="feel">${feel.native}</span>`
                                : ``
                        }
                        </div>`
            case 'emoji':
            ...
            switch (button.dataset.type) {
                case 'sticker':
                    button.onmouseup = (e) => {
                        if (
                            !stickerBox ||
                            stickerBox.classList.contains('hidden')
                        ) {
                            e.stopPropagation()
    ...
    async function handleShowSticker(e, targetEl) {
        ...
        setTimeout(() => {
            ...
            let top =
                rectTargetElement.top - rect.height - rectTargetElement.height
            ...
            let excessPart =
                sumLeftAndWidthStickerBox -
                rectTargetElement.left -
                rectTargetElement.width / 2
        ...
        listTab.forEach((tab, index) => {
            tab.onclick = async (e) => {
                ...
                let items = await Promise.all(
                    listStickers[tab.dataset.index].items
                        .map((item) => {
    ...
    function handleKeyup(e) {
        ...
        if (
            oldContent.slice(oldContent.length - 2, oldContent.length - 1) ===
            ':'
        ) {
        ...
        inputFile.addEventListener('change', (e) => {
            ...
            Array.from(e.target.files).forEach((file) => {
                ...
                image.onload = function () {
                    let randomString =
                        Math.random().toString(36).toString(36).substring(2) +
    ...
    function sortDataUpload() {
        ...
        for (const fileEl of listFileAddEl.children) {
            ...
            const index = Array.from(newDataTransfer.files).findIndex((file) =>
                isSameFile(file, info)
            )
            if (index !== -1) {
    ...
    function isSameFile(file, infoFile) {
        return (
            ...
            file.size === infoFile.size &&
            file.customId === infoFile.customId &&
            file.type === infoFile.type
        )
    }
    return {
        init: () => {
