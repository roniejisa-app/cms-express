...
function handleUploadPlace() {
        ...
        input.onchange = (e) => {
            ...
            if (dataTransferFiles.length === 0) {
                ...
                const newFiles = Array.from(files).filter((file) => {
                    if (
                        !Array.from(dataTransferFiles).some(
    ...
    function renderItems(items) {
        const listEl = Array.from(items).map((file, index) => {
            ...
            if (
                ['png', 'jpeg', 'jpg', 'webp', 'tiff', 'svg', 'bmp'].includes(
                    extension.toLowerCase()
                )
            ) {
                img.src = url
            } else if (
                ['docx', 'pptx', 'xlsx', 'pdf', 'mp4'].includes(
                    extension.toLowerCase()
                )
            ) {
        ...
        listEl.forEach((item) => {
            item.button.onclick = (e) => {
                ...
                const indexRemove = Array.from(dataTransfer.files).findIndex(
                    (dataTransferFile) => isSame(dataTransferFile, item)
                )
...
export { eventAddFolder, eventUploadFile, eventLoadItem }
