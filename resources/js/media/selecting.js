    ...
    if (selecting) {
        ...
        if (event.target === canvas) {
            ...
            Array.from(items).forEach((item) => {
                if (
                    isCollision(item, divCloneCanvas) &&
                    event.ctrlKey &&
                    itemsSelecting.includes(item)
                ) {
                    item.firstElementChild.checked = false
                } else if (
                    isCollision(item, divCloneCanvas) ||
                    (event.ctrlKey && itemsSelecting.includes(item)) ||
                    (event.shiftKey && itemsSelecting.includes(item))
                ) {
                ...
                if (listItemSelecting.length) {
                    ...
                    if (
                        ...
                        for (
                            ...
                            listItemSelecting[i].firstElementChild.checked =
                                i <= _this.index
                        }
                    } else if (
                        _this.index <= lastItemChecked.index &&
                    ...
                    Array.from(items)
                        ...
                        .forEach(
                            (item) => (item.firstElementChild.checked = true)
                        )
                }
                /**
    ...
    */
    ...
    document.addEventListener('mousedown', function (event) {
        if (
            ...
            getItemSelecting().forEach(
                (item) => (item.firstElementChild.checked = false)
            )
        }
    })
    ...
    window.addEventListener('show-info-file', function () {
        ...
        if (objectItem.index === null || objectItem.index === undefined) {
            mediaInfo.innerHTML =
                '<p class="text-center">Chọn tệp để xem thông tin</p>'
            return false
        }
        ...
        if (custom.extension && custom.pathAbsolute) {
            if (
                ['png', 'gif', 'jpg', 'webp', 'jpeg', 'svg'].includes(
                    custom.extension
                )
            ) {
                ...
                ['docx', 'mp4', 'pdf', 'xlsx', 'pptx'].includes(
                    custom.extension
                )
            ) {
