const o=function(e=!1){const t=document.createElement("div");return t.className="item",t.innerHTML=`${e?`<div class="image">
                        ...
                        </div>`,t},s=function(){const e=document.createElement("div");return e.className="variant-item",e.innerHTML=`<div class="product-variant-items">
        ...
        </div>`,e};class n{constructor(t){this.variantOptions=[],this.element=t,this.productVariantSwitch=this.element.querySelector("[switch-product-variant] input"),this.productNoVariant=this.element.querySelector("[product-no-variant]"),this.productVariant=this.element.querySelector("[product-has-variant]"),this.productVariantOptions=this.element.querySelector("[product-variant-options]"),this.variantList=this.element.querySelector("[product-variant-list]"),this.addVariant=this.productVariant.querySelector("[add-variant]")}toggleVariant(){this.productVariantSwitch.addEventListener("change",t=>{this.productVariantSwitch.checked?(this.productNoVariant.style.display="none",this.productVariant.style.display="block"):(this.productNoVariant.style.display="flex",this.productVariant.style.display="none")})}getItems(){this.variantOptions=[];for(let t of this.variantList.children){const i=new r(t);this.variantOptions.push(i)}this.buildData()}checkLength(){return!(this.variantOptions.length>=3)}addItem(){this.addVariant.onclick=()=>{if(!this.checkLength())return!1;const t=s();this.variantList.append(t),this.getItems(),this.checkLength()||(this.addVariant.style.display="none")}}createTable(){this.table=document.createElement("table"),this.table.className="table",this.tableHead=document.createElement("thead"),this.tableHead.innerHTML=`<tr>
        ...
        </tr>`,this.table.append(this.tableHead,this.body),this.productVariantOptions.append(this.table)}buildData(t){}}class r{constructor(t){this.item=t,this.name=null,this.options=[],this.init()}init(){var t;this.itemHasImage=((t=this.item.querySelector("[has-image]"))==null?void 0:t.checked)||!1,this.itemOptions=this.item.querySelector("[variant-options] .items"),this.initOptions()}getName(){}initOptions(){for(let t of this.itemOptions.children){const i=t.querySelector("input");let a=null;i.oninput=()=>{clearTimeout(a),a=setTimeout(()=>{i.value!=""&&(this.itemOptions.append(o(this.itemHasImage)),this.initOptions())},1e3)}}}}const l=document.querySelectorAll("[product-variant]");for(const e of l){const t=new n(e);t.toggleVariant(),t.addItem(),t.getItems(),t.createTable(),t.buildData()}
